...
import {assertInInjectionContext, inject, provideAppInitializer, runInInjectionContext} from '@angular/core';
...
import type {Client, Config, RequestOptions, ResolvedRequestOptions, ResponseStyle} from './types.gen';
import {buildUrl, createConfig, createInterceptors, mergeConfigs, mergeHeaders, setAuthParams} from './utils.gen';
  ...
  const request: Client['request'] = async (options) => {
    ...
    try {
      ...
      result.response = await firstValueFrom(
        opts.httpClient!.request(req).pipe(filter((event) => event.type === HttpEventType.Response)),
      );
      ...
      for (const fn of interceptors.error.fns) {
        if (fn) {
          finalError = await fn(finalError, result.response, result.request, options as ResolvedRequestOptions);
        }
      }
