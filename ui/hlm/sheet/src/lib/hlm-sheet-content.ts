...
@Component({
  ...
  template: `
    ...
    @if (showCloseButton()) {
      <button class="absolute end-3 top-3" hlmBtn hlmSheetClose size="icon-sm" variant="ghost">
...
export class HlmSheetContent {
  ...
  readonly state = this._stateProvider.state ?? signal('closed');
  private readonly _renderer = inject(Renderer2);
  private readonly _element = inject(ElementRef);

  readonly showCloseButton = input<boolean, BooleanInput>(true, {transform: booleanAttribute});
